...
% 
% % vector to centroids
% centFull=ones(size(CCLyrics)).*rw/2;
% CCLyricsVec=CCLyrics-centFull;
% lenLyricsCC=length(centFull);
% CCLyricsVecNew=[CCLyricsVec,zeros(length(CCLyricsVec),1)];
% 
% centFull=ones(size(CCfeat)).*rw/2;
% CCfeatVec=CCfeat-centFull;
% lenfeatCC=length(CCfeatVec);
% CCfeatVecNew=[CCfeatVec,zeros(length(CCfeatVec),1)];
% 
% % min cross is match
% VecProds=zeros(length(CCLyricsVec),length(CCfeatVec));
% for i=1:length(CCLyricsVec)
%     for j=1:length(CCfeatVec)
%         VecProds(i,j)=cross(CCLyricsVecNew(i,:),CCfeatVecNew(j,:));
%     end
% end
% 
% for i=1:length(CCLyricsVec)
%     [~,VecProdLoc]=min(VecProds(i,:));
% end